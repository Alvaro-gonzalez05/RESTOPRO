...
export default function PuntosConfigPage() {
  ...
  const [productos, setProductos] = useState<{id:number, name:string, puntos:number, canjeo?:number}[]>([])
  ...
  useEffect(() => {
    async function fetchAll() {
      ...
      setProductos(prods.map((p: any) => {
        const conf = config.find((c: any) => c.product_id === p.id)
        return {
          id: p.id,
          name: p.name,
          puntos: conf ? (conf.points || 0) : 0,
          canjeo: conf ? (conf.redeem_points || 0) : 0
        }
      }))
  ...
  const handleCanjeoProducto = (id: number, canjeo: number) => {
    setProductos(productos.map(p => p.id === id ? { ...p, canjeo } : p))
  }
  ...
  const handleGuardar = async () => {
    ...
    await Promise.all(productos.map(p => upsertPointsConfig({ product_id: p.id, points: p.puntos, redeem_points: p.canjeo })))
                  ...
                  <TableRow key={producto.id}>
                    ...
                    <TableCell>
                      <Input
                        type="number"
                        min={0}
                        value={producto.canjeo || 0}
                        onChange={e => handleCanjeoProducto(producto.id, Number(e.target.value))}
                        className="w-24 rounded-[30px]"
                      />
                    </TableCell>
                  </TableRow>
