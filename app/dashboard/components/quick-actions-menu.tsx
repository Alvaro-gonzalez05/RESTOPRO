...
import { CreateExpenseDialog } from "@/app/dashboard/gastos/components/create-expense-dialog"
...
export function QuickActionsMenu() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Prevenir la acción por defecto solo para nuestras teclas específicas
      if (e.key === 'F5' || e.key === 'F6' || e.key === 'F7' || e.key === 'F8') {
        e.preventDefault()
        
        switch (e.key) {
          case 'F5':
            // Nueva Venta - redirigir a órdenes
    ...
    <Card className="mb-8">
      <CardHeader>
        ...
        <p className="text-sm text-gray-600">
          Accede rápidamente a las funciones principales o usa las teclas F5-F8
        </p>
      </CardHeader>
      <CardContent>
      ...
      <CreateSupplierQuickDialog 
        ...
        onOpenChange={(open) => setShowSupplierDialog(open)}
